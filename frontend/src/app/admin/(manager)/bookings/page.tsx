...
export default function AdminBookingsPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-foreground mb-2">ตรวจสอบการชำระเงิน</h1>
        <p className="text-muted-foreground">
          ตรวจสอบและอนุมัติหลักฐานการโอนเงินจากลูกค้า
        </p>
      </div>
      ...
      {!proofs || proofs.length === 0 ? (
        <div className="bg-muted/30 rounded-lg p-12 text-center">
          ...
          <p className="text-muted-foreground">
            {filter === 'pending' ? 'ไม่มีหลักฐานการชำระเงินที่รอตรวจสอบ' : 'ไม่มีหลักฐานการชำระเงิน'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {proofs.map((proof) => (
            <div key={proof.payment_proof_id} className="bg-background rounded-lg shadow-lg overflow-hidden border border-border">
              {/* Header */}
              <div className="bg-muted/50 px-6 py-4 border-b border-border">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-semibold text-foreground">การจอง #{proof.booking_id}</h3>
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    ...
                    {proof.status === 'pending' ? 'รอตรวจสอบ' : proof.status === 'approved' ? 'อนุมัติแล้ว' : 'ปฏิเสธ'}
                  </span>
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-foreground">
                      {formatDate(proof.check_in_date, 'dd MMM')} - {formatDate(proof.check_out_date, 'dd MMM yyyy')}
                    </p>
                  </div>
                  <div>
                    <p className="text-muted-foreground mb-1">วิธีชำระเงิน</p>
                    <p className="font-medium text-foreground">
                      {proof.payment_method === 'bank_transfer' ? 'โอนเงินผ่านธนาคาร' : 
                       proof.payment_method === 'qr_code' ? 'QR Code พร้อมเพย์' : 
                       proof.payment_method}
                    </p>
                  </div>
                  <div>
                    <p className="text-muted-foreground mb-1">จำนวนเงิน</p>
                    <p className="font-bold text-primary text-lg">{formatCurrency(proof.amount)}</p>
                  </div>
                </div>
      ...
      {selectedProof && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" onClick={() => setSelectedProof(null)}>
          <div className="bg-background rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-background border-b border-border px-6 py-4 flex items-center justify-between">
              <h3 className="text-xl font-bold text-foreground">หลักฐานการโอนเงิน - การจอง #{selectedProof.booking_id}</h3>
              <button
