 ...
 * Proxy to backend room type detail API
 */
export async function GET(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[Room Type Detail Proxy] Backend error:', response.status, errorText);
    ...
    console.error('[Room Type Detail Proxy] Error:', error);
    return NextResponse.json(
      {
        success: false,
